...
pub fn invoke_java_entry(env: &JNIEnv) {
...
pub fn find_class<'a>(env: &'a JNIEnv, class_name: String) -> anyhow::Result<JClass<'a>> {
    let mut loader_guard = DEX_LOADER.lock().unwrap();
    let result = loader_guard.find_class_from_hooker_class_loader(env,class_name);
    match result{
        Ok(c) => {
            Ok(c)
        }
...
impl DexLoader {
    ...
    pub fn get_class_loader(&mut self) -> &Option<GlobalRef> {
        &self.class_loader
    }
    ...
    pub fn load_dex_file(&mut self,env: &JNIEnv) -> Result<(), Error>{
    ...
    fn find_class_from_hooker_class_loader<'a>(&mut self, env: &JNIEnv<'a>, class_name: String) -> Result<JClass<'a>,Error> {
    ...
    fn register_necessery_functions(&mut self,env: &JNIEnv) -> anyhow::Result<()> {
    ...
    #[allow(dead_code)]
    pub fn invoke_java_entry(&mut self, env: &JNIEnv) {
    ...
    pub fn register_hooker_native_methods(&mut self,env: &JNIEnv,native_methods: Vec<jni::NativeMethod>, class_name: String) -> anyhow::Result<()>{
